...
public class Database {
   ...
   */
  public byte[] getAttachment(String id, String attachment) throws IOException {
	    CouchResponse resp = session.get(name + "/" + urlEncodePath(id) + "/" + attachment);
	    return IOUtils.toByteArray(resp.getContent());
   ...
   * @param attachment attachment body
   * @return was the delete successful? CouchDB JSON response
   */
    public String putAttachment(String id, String rev, String fname, String ctype, InputStream attachment, long length) throws IOException {
        CouchResponse resp = session.put(name + "/" + urlEncodePath(id) + "/" + fname, rev, ctype, attachment, length);
        return resp.getBody();
    } 
}
